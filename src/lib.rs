...
pub fn luaproc(input: TokenStream) -> TokenStream {
    ...
    let (lua_meta, lua_script) = LuaMeta::new(&ast);
    // lua_meta
    ...
    match &ast.data {
        Data::Struct(ds) => {
            let lua_struct = LuaStruct::new(lua_meta, ds);

            Lua::lua_set_var(&lua, &globals, &lua_struct, "struct")
                .unwrap_or_else(|e| panic!("Lua {e}"));
        }
        Data::Enum(de) => {
            let lua_enum = LuaEnum::new(lua_meta, de);
    ...
    // exec lua code
    Lua::lua_exec_code(&lua, lua_script).unwrap_or_else(|e| panic!("Lua {}", e));
    ...
    if let Ok(code) = globals.get::<_, String>("code") {
        ...
        let expr = syn::parse_str::<proc_macro2::TokenStream>(&code).unwrap_or_else(|_| {
            panic!("injected code '{code}' from Lua is not a well-formed Rust code")
        });
